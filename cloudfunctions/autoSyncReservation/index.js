  ...
  try {
    ...
    for (const r of reservations) {
      ...
      if (!purchaseRes.data || purchaseRes.data.length === 0) {
        // Read approval rules before creating purchase
        var rulesRes = await db.collection('settings').where({ key: 'approval_rules' }).get()
        var rules = (rulesRes.data && rulesRes.data.length > 0) ? (rulesRes.data[0].value || {}) : {}

        // Check if banquet category needs approval
        ...
        // If approval needed, set default approver from rules
        if (needApproval) {
