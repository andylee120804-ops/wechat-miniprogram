var app = getApp()
var { formatDate, formatDateTime, formatAmount, getCategoryName, getReservationStatusText, getExclusiveTypeName } = require('../../utils/helpers')
...
var { deleteIncomeByPurchase } = require('../reservation-add/helpers/sync')
...
Page({
  ...
  loadPurchase: async function(id) {
    ...
    db.getDoc(COLLECTIONS.PURCHASE, id).then(async function(data) {
      ...
      // Admin/boss can delete any purchase record
      var userRole = (userInfo.role || '').toLowerCase()
      var isAdmin = userRole === 'admin' || userRole === 'boss'
      ...
      // Load linked reservation for banquet purchases
      if (data.sourceReservationId) {
        that.loadLinkedReservation(data.sourceReservationId)
      } else {
        that.setData({ reservation: null })
      }
  ...
  loadApprovalLogs: function(purchaseId) {
    ...
    db.queryAll(COLLECTIONS.APPROVAL_LOG, { purchaseId: purchaseId }, 'createdAt', 'asc').then(function(result) {
      ...
      } else if (status === 'approved' && purchase.autoGenerated && purchase.approverName === '宴会创建自动批复') {
        // Auto-approved by banquet creation
        approvalStep = {
          badge: '通过', badgeBg: '#22C55E',
          text: '宴会创建自动批复',
          time: purchase.formattedCreatedAt || ''
        }
  ...
  loadLinkedReservation: function(reservationId) {
    var that = this
    db.getDoc(COLLECTIONS.RESERVATION, reservationId).then(function(resData) {
      if (!resData) {
        that.setData({ reservation: null })
        return
      }
      var et = resData.exclusiveType || (resData.isExclusive ? 'full' : 'none')
      var reservation = {
        ...resData,
      ...
      that.setData({ reservation: reservation })
    }).catch(function(err) {
      console.warn('加载关联预约失败:', err)
      that.setData({ reservation: null })
    })
  },
  ...
  onDelete: function() {
    this.setData({ showDeleteModal: true, deleteReason: '' })
  },

  onDeleteReasonInput: function(e) {
  ...
  onDeleteConfirm: async function() {
    ...
    var reason = that.data.deleteReason || ''
    if (!reason.trim()) {
    ...
    if (!that.data.canDelete && !isAdmin) {
    ...
    // Log the deletion with reason to operation_log
    log(LOG_TYPES.PURCHASE_DELETE, '删除采购: ' + (that.data.purchase ? that.data.purchase.item : ''), {
      id: that.data.id,
      reason: reason.trim(),
    ...
    // Also write an approval log for audit trail
    db.addDoc(COLLECTIONS.APPROVAL_LOG, {
      purchaseId: that.data.id,
      action: 'deleted',
      operatorId: userInfo._id || '',
      ...
      console.warn('记录删除日志失败:', e)
    })

    // Clean up linked income for banquet purchases before deleting
    var purchase = that.data.purchase || {}
    if (purchase.category === 'banquet' && purchase.sourceReservationId) {
      deleteIncomeByPurchase(that.data.id, purchase.sourceReservationId).catch(function(e) {
        console.warn('[purchase-detail] 删除关联收入失败:', e)
      })
    }
    ...
    db.deleteDoc(COLLECTIONS.PURCHASE, that.data.id).then(function() {
      ...
      // Clean up associated approval logs (except the delete log we just wrote)
  ...
  onDeleteCancel: function() {
    this.setData({ showDeleteModal: false, deleteReason: '' })
  },
