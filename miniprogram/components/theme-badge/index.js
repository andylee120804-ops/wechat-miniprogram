  ...
  observers: {
    'status, type, theme': function(status, type, theme) {
      if (type === 'status') {
        ...
        const key = status || this.data.text
        const color = colors[key] || '#9CA3AF'
        this.setData({ bgColor: color + '22', textColor: color })
