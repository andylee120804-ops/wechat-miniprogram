...
| `miniprogram/pages/reservation-detail/index.wxml` | 弹窗中新增文字模板选择器 UI |
...
## Task 3: reservation-detail 分享弹窗新增文字模板选择器
...
onShareToGuest() {
  ...
  this.setData({
    ...
    selectedTemplate: saved.template || 'business'
  })
}
```
...
修改 `_buildShareConfig` 保存 template：
...
在 `theme-modal` 弹窗中，标题下方添加文字模板选择器：

```wxml
<view style="display:flex; align-items:center; margin-bottom:16rpx;">
  <text style="color:{{selectedTemplate==='business'?'#C9A96E':'#5C5C72'}};font-size:26rpx;font-weight:{{selectedTemplate==='business'?'600':'400'}};" data-id="business" bindtap="onTemplateSelect">商务风格</text>
  <text style="color:#5C5C72;font-size:26rpx;margin:0 16rpx;">|</text>
  <text style="color:{{selectedTemplate==='friend'?'#C9A96E':'#5C5C72'}};font-size:26rpx;font-weight:{{selectedTemplate==='friend'?'600':'400'}};" data-id="friend" bindtap="onTemplateSelect">友情风格</text>
</view>
```

- [ ] **Step 6: 修改 index.js — 添加 selectedTemplate、onTemplateSelect、修改 _buildShareConfig**

- [ ] **Step 7: 修改 index.wxml — 在弹窗标题下方添加文字模板选择器**

- [ ] **Step 8: 提交**

```bash
git add miniprogram/pages/reservation-detail/index.js miniprogram/pages/reservation-detail/index.wxml
git commit -m "feat(detail): 分享弹窗新增文字模板选择器"
```
...
5. 默认值验证：从未保存过模板的预约，打开弹窗后默认选中商务风格