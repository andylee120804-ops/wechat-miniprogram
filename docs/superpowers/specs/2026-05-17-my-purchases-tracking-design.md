...
### 修改文件
- `miniprogram/pages/me/index.js` — 新增入口项、`pendingGroup` 数据和路由
- `miniprogram/pages/me/index.wxml` — 待办分组从硬编码单项改为 `wx:for="{{pendingGroup}}"` 动态渲染（原待办事项 + 新增采购申请入口）
...
> **性能兜底：** 若单用户采购记录超过 300 条（当前业务场景不太可能），后续可引入虚拟列表或分页加载。当前方案优先保证状态切换的即时体验。
...
| 场景 | 表现 |
|------|------|
| 用户未登录 / 登录态过期 | `onShow` 中检测 `currentUserId` 缺失时，直接 `wx.redirectTo('/pages/login/index')` 跳转登录页 |
