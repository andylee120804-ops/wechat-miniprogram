...
// Mock getApp for sync methods that call it
  ...
  const instance = {
    ...
    async syncBanquetPurchase(docData, reservationId, isCreate) {
      try {
        ...
        console.error('[TEST ERROR] syncBanquetPurchase:', err)
      }
    },
    ...
    async deleteBanquetPurchase(reservationId) {
      try {
        ...
        for (var i = 0; i < (purchases.data || []).length; i++) {
          await mockDb.deleteDoc(COLLECTIONS.PURCHASE, purchases.data[i]._id)
        }
        ...
        for (var j = 0; j < (incomes.data || []).length; j++) {
          await mockDb.deleteDoc(COLLECTIONS.INCOME, incomes.data[j]._id)
        }
      } catch (err) {
        // silent
      }
